...
5 Input "ENTER AS HOUR (1-24), MINUTES (1-59) - USE COMMA: ", HH, TM
...
15 T1 = 0: T2 = 0: T3 = 0
16 SC = 0: MI = 0: Y = 0 ' Initialize time counters
17 If A1$ = "S" Then T1 = 6 Else GoTo 21
19 SC = SC + T1: If SC >= 60 Then SC = SC - 60: TM = TM + 1

21 If A1$ = "R" Then T2 = 1: MI = MI + T2 Else GoTo 25

25 If A1$ = "T" Then T3 = 10: TM = TM + T3
27 If (TM + MI) >= 60 Then TM = TM - 60: HH = HH + 1
29 If HH >= 24 Then HH = 0

33 Print: Print "TIME IN ADVENTURE "; HH; ":"; (TM + MI); "."; SC: Print

35 If (TM + MI) > 29 And (TM + MI) < 40 Then Y = 1
37 If (TM + MI) >= 0 And (TM + MI) < 10 Then Y = 1
...
50 M = Int(Rnd * 6 + 1)
51 If M < 6 Then GoTo 11 ' Wandering monster appears only on a 6

60 Print: Input "LEVEL BELOW SURFACE: ", L
61 Restore: P = Int(Rnd * 12)
70 On L GOTO 1000, 2000, 3000

80 Q = Int(Rnd * 12)
81 For I = 1 To Q: Read M$, N, T, H, A, C, D, S: Next I

90 B = Int(Rnd * N) + 1 ' Guarantee at least one appears
91 B = B * T

100 Print M$
101 Print "NUMBER APPEARING: "; B
110 Print "HIT POINTS: ";
111 For I = 1 To B: Print Int(Rnd * H) + A;: Next I: Print
...
160 R = Int(Rnd * 6 + 1) + Int(Rnd * 6 + 1) ' 2d6 roll
...
' Level 1 table jumps (1-3 HD creatures)
...
4210 Data "PIERCER",6,1,8,0,3,6,0,"CARRION CRAWLER",1,1,18,1,7,0,0
